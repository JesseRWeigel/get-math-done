...
statusline with: GMD | model | phase/plan progress | context bar
...
def get_phase_info() -> str:
    ...
    # Look for .gmd directory
    cwd = os.getcwd()
    search = cwd
    while search != os.path.dirname(search):
        state_json = os.path.join(search, ".gmd", "state.json")
...
def format_statusline(data: dict) -> str:
    ...
    # Brand
    parts.append("\033[1;36mGMD\033[0m")  # bold cyan
